...
api.register = async (req, res) => {
    const user = req.body;
    // TODO - validação dos dados do usuário e enviar resposta ao front end
    // de acordo!
    // Validações necessárias:
    // userName != password
    // password == passwordConfirmation
    // userProfilePhotoUrl iniciando com http:// ou https://
...
api.remove = async (req, res) => {
    const { userName } = req.params;
    const user = await new UserDao(req.db).remove(userName);
    res.json(!!user);
};
...
api.list = async (req, res) => {
    ...
    const usuarios = await new UserDao(req.db).list();

    Promise.all(usuarios).then(completo => {
        res.json(completo);
    });
};
